...
while True:
    ...
    product_url = input("Enter Walmart Canada Product URL (or type STOP to stop scraping): ").strip()
    if product_url.upper() == "STOP":
        print("Stopping the loop—no more data will be collected.")
        break
    ...
    image_url = get_element_attribute(
        '//*[@id="maincontent"]/section/main/div[2]/div[2]/div/div[2]/div/div/section[1]/div[2]/div[1]/img', 
        "src"
    )
    ...
    file_exists = False
    try:
        with open(csv_filename, "r", encoding="utf-8"):
            file_exists = True
